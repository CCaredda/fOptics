...
void PProcessTimes::setNbTotFrames(int v)
{
    ...
    //Consider all frames
    if(_M_activity_time_start.empty())
    {
...
void PProcessTimes::_Compute_Bold_Signal()
{
    ...
    QFile HRF_file(getShareDirPath("share")+"/"+"files/Bold_Signal.txt");
    if(!HRF_file.exists())
    {
        qDebug()<<"PProcessTimes::_Compute_Bold_Signal() Bold_Signal.txt does not exist";
        return;
    }

    QVector<float> HRF_signal;
    if(HRF_file.open(QIODevice::ReadOnly))
    {
        QTextStream in(&HRF_file);
        while(!in.atEnd())
        {
            QString line = in.readLine();
            HRF_signal.push_back( line.toFloat());
        }
        HRF_file.close();
    }
    else
    {
        qDebug()<<"PProcessTimes::_Compute_Bold_Signal() cannot open Bold_Signal.txt ";
        return;
    }

    ///////////////////////
    //Calculate paradigm window function
    ///////////////////////
    QVector<float> y_paradigm(length_BOLD,0);

    //activity periods
    for(int i=0;i<_M_activity_time_start.size();i++)
    {
        for(int frame_idx=_M_activity_time_start[i];frame_idx<_M_activity_time_end[i];frame_idx++)
            y_paradigm[frame_idx] = 1;
    }
    ...
    computeConvolvedSignals(HRF_signal, y_paradigm, (float)_M_FS, _M_BOLD_signal);

    WriteTemporalVector("C:\\Users\\ccaredda\\Desktop\\tmp\\_M_BOLD_signal.txt",_M_BOLD_signal);
    WriteTemporalVector("C:\\Users\\ccaredda\\Desktop\\tmp\\y_paradigm.txt",y_paradigm);
    ...
    qDebug()<<"[Compute Bold] length yparadigm: "<<y_paradigm.size();
