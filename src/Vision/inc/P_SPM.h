...
public:
    ...
    /** Process SPM procedure with automatic thresholding procedure */
    Mat process_SPM_auto_thresh(Mat Delta_C_map, QVector<cv::Point> pixels_pos, cv::Size img_size, QString path, QVector<float> &model, Mat &img_mask, float auto_thresh);
