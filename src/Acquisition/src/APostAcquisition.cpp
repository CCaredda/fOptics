...
void APostAcquisition::run()
{
    ...
    for (;;)
    {
        ...
        _ProcessPreProcessing(img); // heavy work outside the lock
    }
}
...
{
    // qDebug()<<"In  APostAcquisition::newImage(_Processed_img)";
    QMutexLocker locker(&_M_mutex);
...
void  APostAcquisition::newImage(Mat in_img,int frame_id)
{
    ...
    // qDebug()<<"In  APostAcquisition::newImage(Mat in_img,int frame_id)";
    QMutexLocker locker(&_M_mutex);
    _M_img_buffer.enqueue(img); // push to back
    _M_condition.wakeOne(); // wake the sleeping thread
