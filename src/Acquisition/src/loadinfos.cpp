...
QString getShareDirPath(QString key_name)
{
    ...
    for (const QString& candidate : candidates)
    {
        ...
        if (dir.exists())
        {
            qDebug() <<key_name<< "dir found:" << dir.canonicalPath();
            return dir.canonicalPath();
        }
    }

    qWarning() <<key_name<< "dir not found! Searched in:";
    for (const QString& candidate : candidates)
        qWarning() << " -" << QDir::cleanPath(candidate);
...
int preferredBackendForFile(const QString& filePath)
{
    ...
    if (ext == "mkv" || ext == "mp4" || ext == "mov" || ext == "avi" || ext == "mpg" || ext == "mpeg") {
