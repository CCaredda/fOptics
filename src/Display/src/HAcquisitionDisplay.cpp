    ...
    ui(new Ui::HAcquisitionDisplay)
{
    ...
    connect(&_M_load_datas_hard_drive,SIGNAL(requestPreProcessing(Mat)),ui->_ROI,SLOT(init_Interface()));
    ...
    connect(ui->_ROI,SIGNAL(requestAnalysisZoneDrawing()),this,SIGNAL(requestAnalysisZoneDrawing()));
    ...
    //NCC results
    connect(&_M_reg,SIGNAL(registrationIsFinished()),this,SLOT(DisplayRegistrationResult()));
    ...
    //Do not allow ROI buttons at first. Allow button access when an image has been loaded
    ...
    connect(ui->_param_LoadImg,SIGNAL(newVideoReadingMethod(QString)),&_M_load_datas_hard_drive,SLOT(onnewVideoReadingMethod(QString)));
...
void HAcquisitionDisplay::onNew_Guru_Mode(bool v)
{
    ...
    ui->_param_LoadImg->onNew_Guru_Mode(v);
    if(v)
    {
...
void HAcquisitionDisplay::onCheckCamera_ID(bool)
{
...
void HAcquisitionDisplay::newAnalysisZone(QVector<QPoint> c)
{
    ...
    ui->_ROI->setEnabled(false);
    ui->_param_LoadImg->setEnabled(false);
}
...
//display result NCC
void HAcquisitionDisplay::DisplayRegistrationResult()
{

//    emit newRegistrationResults(_M_reg.getNCCResults());

//    if(ui->_mode_RGB_HS->isChecked()) //RGB processing
//        emit newRegistrationResults(_M_reg.getNCCResults());
//    else
//    {
//        //TEMP
//        QVector<QVector<QVector<float> > > NCC = _M_reg_HS.getNCCResults();
//        emit newRegistrationResults(NCC);
//        QVector<QVector<float> > NCC_reg,NCC_no_reg;

//        for(int k=0;k<NCC.size();k++)
//        {
//            NCC_reg.push_back(NCC[k][1]);
//            NCC_no_reg.push_back(NCC[k][0]);
//        }

//        WriteTemporalVector(_M_result_directory+"NCC_no_reg.txt",NCC_no_reg);
//        WriteTemporalVector(_M_result_directory+"NCC_reg.txt",NCC_reg);
//    }
}
...
void HAcquisitionDisplay::onTestRequired()
{
