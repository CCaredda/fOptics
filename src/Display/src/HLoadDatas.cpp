    ...
    ui(new Ui::HLoadDatas)
{
    ...
    //video reading type
    ui->_video_type->addItem("Read video with ffmpeg");
    ui->_video_type->addItem("Read video with Qt");
    ui->_video_type->addItem("Read video with Opencv");
    ui->_video_type->setCurrentIndex(0);
    connect(ui->_video_type,SIGNAL(currentIndexChanged(int)),this,SLOT(onVideoReadingMethodChanged(int)));
...
/** Reading video method changed */
void HLoadDatas::onVideoReadingMethodChanged(int v)
{
    QString method;
    switch (v) {
    case 0:
    ...
    emit newVideoReadingMethod(method);
}


/** set advanced setting method */
void HLoadDatas::onNew_Guru_Mode(bool v)
{
    ui->_video_type->setVisible(v);
}
