...
public:
    ...
    /** Set Wavelength IDX */
    void setWavelengthIDX(QVector<QVector<int> > wavelength_IDX);
...
//    void getAveragedConcentrationChanges(const QVector<QVector<float> > &temporal_vec, const QVector<Mat> &coeff, const QVector<int> &mask, QVector<float> &results);
    ...
    /** Compute the absorbance changes */
    void getAbsorbancechanges(const QVector<QVector<float> > &temporal_vec,int start_ref,int end_ref,int start_id,int end_id,QVector<QVector<float> > &absorbance);
...
public slots:
    ...
    /** Set new wavelength idx */
    void onnewWavelengthIDX(QVector<QVector<int> > wavelength_IDX);

    /** Set indexes of first and last rest steps */
    // void on_newDataCorrectionIdx(int start_first_rest, int end_first_rest,int start_last_rest,int end_last_rest) {_M_start_first_rest=start_first_rest; _M_end_first_rest=end_first_rest; _M_start_last_rest=start_last_rest; _M_end_last_rest=end_last_rest;}
...
//    void _Single_Deconvolution(const QVector<QVector<float> > &temporal_vec, const Mat &coeff, const QVector<int> &mask, QVector<float> &results,int start_act_id,int end_act_id);

//    void _Multiple_Deconvolution(const QVector<QVector<float> > &temporal_vec, const QVector<Mat> &coeff, const QVector<int> &mask, QVector<float> &results);
    void _Multiple_Deconvolution(const QVector<QVector<float> > &temporal_vec, const QVector<Mat> &coeff, QVector<float> &results, int start_act_id=-1, int end_act_id=-1, int start_ref=-1, int end_ref=-1);
    ...
    void _Multiple_Deconvolution_Time_courses(const QVector<QVector<float> > &temporal_vec, const QVector<Mat> &coeff, QVector<QVector<float> > &contrast,int start_ref,int end_ref,int start_id,int end_id);


    //Wavelength IDX
    //size(Nb deconvolution system,Nb Channels
    QVector<QVector<int> > _M_wavelength_IDX;

    //Coeff multi (for correlation)
    QVector<int> _M_coeff_multi;
