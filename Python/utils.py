...
def remove_small_contour(fOptics_mask, reso_mm, min_area):
    ...
    for i in range(len(c)):
        area_px = cv2.contourArea(c[i])
        area_mm2 = area_px * (reso_mm ** 2)
...
def get_mean_std_distance_to_EBS2(ROIs, idx_epicenter, fOptics_mask, reso_mm, Thresh_activity, contour_function):
    ...
    if dst_vec.shape[0]>0:
        ...
        dist2 = dst_vec[dst_vec>0]
        if dist2.shape[0]>0:
            m_practical=int(100*dist2.mean())/100
            nb = dist2.shape[0]
        else:
            m_practical=np.nan
            nb = 0
...
def convert_dist_sensi_values_in_str(arr, nb_detec, nb_tot):
...
def write_metric_results_Latex(filename, np_metric_auto, np_metric_SPM,
    ...
    "Sensitivity motor auto": convert_dist_sensi_values_in_str(100*np_metric_auto.Sensitiviy_motor,
                                                               np_metric_auto.TP_motor.sum(),
                                                               np_metric_auto.nb_EBS_motor.sum()),
    "Dist motor auto (mm)": convert_dist_sensi_values_in_str(np_metric_dist_auto.mean_practical_sensi_motor,
                                                             np_metric_dist_auto.tot_motor_areas.sum(),
                                                             np_metric_auto.nb_EBS_motor.sum()),

    "Sensitivity motor SPM": convert_dist_sensi_values_in_str(100*np_metric_SPM.Sensitiviy_motor,
                                                               np_metric_SPM.TP_motor.sum(),
                                                               np_metric_auto.nb_EBS_motor.sum()),
    "Dist motor SPM (mm)": convert_dist_sensi_values_in_str(np_metric_dist_SPM.mean_practical_sensi_motor,
                                                            np_metric_dist_SPM.tot_motor_areas.sum(),
                                                            np_metric_auto.nb_EBS_motor.sum()),


    "Sensitivity sensory auto": convert_dist_sensi_values_in_str(100*np_metric_auto.Sensitiviy_sensory,
                                                               np_metric_auto.TP_sensory.sum(),
                                                               np_metric_auto.nb_EBS_sensory.sum()),
    "Dist sensory auto (mm)": convert_dist_sensi_values_in_str(np_metric_dist_auto.mean_practical_sensi_sensory,
                                                               np_metric_dist_auto.tot_sensory_areas.sum(),
                                                               np_metric_auto.nb_EBS_sensory.sum()),

    "Sensitivity sensory SPM": convert_dist_sensi_values_in_str(100*np_metric_SPM.Sensitiviy_sensory,
                                                               np_metric_SPM.TP_sensory.sum(),
                                                               np_metric_auto.nb_EBS_sensory.sum()),
    "Dist sensory SPM (mm)": convert_dist_sensi_values_in_str(np_metric_dist_SPM.mean_practical_sensi_sensory,
...
def create_json_patient(json_file, metric_auto = Sensitivity_specificity_metrics(),
    ...
    "nb non detected motor auto": [dist_auto.tot_motor_areas],
    "nb non detected motor SPM": [dist_SPM.tot_motor_areas],
    "nb non detected sensory auto": [dist_auto.tot_sensory_areas],
    "nb non detected sensory SPM": [dist_SPM.tot_sensory_areas],
