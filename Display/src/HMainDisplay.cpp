    ...
    ui(new Ui::HMainDisplay)
{
    ...
    connect(ui->_params_acquisition,SIGNAL(AnalysisChoice(int)),ui->_save_widget,SLOT(AnalysisChoice(int)));
...
//    this->showMaximized();
    ...
    /*****************************************************************************************************
     *****************************************************************************************************
     *******************ACQUISITION : CAMERA OR IMG FROM HARD DRIVE***************************************
     *****************************************************************************************************
     ****************************************************************************************************/

    //Request acquisition info
    connect(ui->_params_acquisition,SIGNAL(requestAcquisitionInfo(bool)),ui->_params_analysis,SIGNAL(requestAcquisitionInfo(bool)));

    //Send acquisition info
    connect(ui->_params_analysis,SIGNAL(newAcquisitionInfo(QString)),ui->_params_acquisition,SLOT(onNewAcquisitionInfo(QString)));

    //Hyperspectral config
    connect(ui->_params_acquisition,SIGNAL(new_HS_config(int)),ui->_params_analysis,SIGNAL(new_HS_config(int)));
    connect(ui->_params_acquisition,SIGNAL(newRGBMode(bool)),ui->_displayResult,SLOT(onnewRGBMode(bool)));

    //Hyperspectral spectral range
    connect(ui->_params_acquisition,SIGNAL(newSpectralRange(int,int)),ui->_params_analysis,SIGNAL(newSpectralRange(int,int)));
    connect(ui->_params_acquisition,SIGNAL(requireSpectralCorrection(bool)),ui->_params_analysis,SIGNAL(requireSpectralCorrection(bool)));
    ...
    //Line drawing
    connect(ui->_params_acquisition,SIGNAL(RequestLinePlot()),ui->_displayResult,SIGNAL(RequestLineDrawing()));
    connect(ui->_displayResult,SIGNAL(newLine(QPoint,QPoint)),ui->_params_acquisition,SIGNAL(newLine(QPoint,QPoint)));
    ...
    connect(ui->_params_analysis,SIGNAL(AnalysisZoneRequested(int)),ui->_displayResult,SIGNAL(onAnalysisZoneRequested(int)));
    ...
    //Send automatic analysis zone
    connect(ui->_params_acquisition,SIGNAL(newAutomaticAnalysisZone(QVector<QPoint>,cv::Size)),this,SLOT(onNewAutoAnalysisZone(QVector<QPoint>,cv::Size)));

    //request ROI drawing
    connect(ui->_params_acquisition,SIGNAL(requestAnalysisZoneDrawing(int)),ui->_params_analysis,SLOT(onAnalysisZoneRequested(int)));
    ...
    connect(ui->_save_widget,SIGNAL(requestNonFilteredDataSaving(bool)),ui->_params_analysis,SIGNAL(requestNonFilteredDataSaving(bool)));
    ...
    connect(ui->_params_analysis,SIGNAL(DataAcquisitionIsReady()),ui->_save_widget,SLOT(onDataAcquisitionIsReady()));
    ...
    connect(ui->_params_acquisition,SIGNAL(newProcessImage_HS(_Processed_img_HS)),ui->_params_analysis,SLOT(onNewProcessImage_HS(_Processed_img_HS)));
    ...
    connect(ui->_params_analysis,SIGNAL(newActivationMap(QVector<bool>,int)),ui->_displayResult,SIGNAL(newActivationMap(QVector<bool>,int)));
    connect(ui->_params_analysis,SIGNAL(newActivationMap(Mat)),ui->_displayResult,SLOT(onNewActivationMap(Mat)));
    ...
    connect(ui->_displayResult,SIGNAL(GreyOutsideContourIsRequested(bool)),ui->_params_analysis,SLOT(onGreyOutsideContourIsRequested(bool)));
    ...
    //Correlation thresh
    connect(ui->_displayResult,SIGNAL(newCorrelationThreshold(double)),ui->_params_analysis,SIGNAL(newCorrelationThreshold(double)));
    ...
    ui->_plot->setPlotNames(names,0);
    ui->_plot->setYName("Concentration variation",0);
    ui->_plot->setXName("Time (s)",0);

    //X interval pour les graphes de conecntrations
    ui->_plot->setXInterval(1,0);
    ui->_plot->setXStart(0,0);
...
//    connect(ui->_params_analysis,SIGNAL(requestStatsZoneInit()),ui->_displayResult,SIGNAL(requestStatsZoneInit()));

    //Statistic mask
    connect(ui->_displayResult,SIGNAL(newCorticalAreaDefinition(Rect,QVector<Rect>,QVector<Mat>)),ui->_params_analysis,SLOT(onnewCorticalAreaDefinition(Rect,QVector<Rect>,QVector<Mat>)));
    connect(ui->_displayResult,SIGNAL(newCorticalAreaDefinition(QVector<Rect>,QVector<Mat>)),ui->_params_analysis,SLOT(onnewCorticalAreaDefinition(QVector<Rect>,QVector<Mat>)));
    ...
    connect(ui->_params_analysis,SIGNAL(enableRestingState(bool)),ui->_displayResult,SLOT(onenableRestingState(bool)));

    //init seeds
    connect(ui->_params_analysis,SIGNAL(requestSeedInit()),ui->_displayResult,SIGNAL(requestSeedInit()));
    connect(ui->_params_analysis,SIGNAL(requestSeedInit()),ui->_params_analysis,SLOT(InitRestingStateSeeds()));

    //Seed radius
    connect(ui->_params_analysis,SIGNAL(newSeedRadius(double)),ui->_displayResult,SIGNAL(newSeedRadius(double)));

    //Grid seed
    connect(ui->_params_analysis,SIGNAL(newGridSeeds(bool)),ui->_displayResult,SIGNAL(newGridSeeds(bool)));

    //request seed
    connect(ui->_params_analysis,SIGNAL(requestSeeds()),ui->_displayResult,SIGNAL(requestSeeds()));

    //new seeds
    connect(ui->_displayResult,SIGNAL(newRestingStateSeeds(QVector<Mat>)),ui->_params_analysis,SLOT(onnewRestingStateSeeds(QVector<Mat>)));

    //Resting states maps
    connect(ui->_params_analysis,SIGNAL(newRestingStateMaps(QVector<QVector<Mat> >)),ui->_displayResult,SLOT(onnewRestingStateMaps(QVector<QVector<Mat> >)));
    connect(ui->_params_analysis,SIGNAL(newKmeans_RestingStateMaps(QVector<Mat>)),ui->_displayResult,SLOT(onnewKmeans_RestingStateMaps(QVector<Mat>)));

    //Request seed connexion
    connect(ui->_params_analysis,SIGNAL(requestSeedConnexion(bool)),ui->_displayResult,SLOT(onSeedConnexionRequested(bool)));

    //Seed extraction: Thee new seed become the resting state map in which the seed is included
    connect(ui->_params_analysis,SIGNAL(requestSeedsExtraction()),ui->_displayResult,SLOT(onrequestSeedsExtraction()));

    //New resting state method (0: seed based method, 1: ICA based method)
    connect(ui->_params_analysis,SIGNAL(newRestingStateMethod(int)),ui->_displayResult,SLOT(onnewRestingStateMethod(int)));


    //new ICA source of interest
    connect(ui->_params_analysis,SIGNAL(newICASourceofInterest(int)),ui->_displayResult,SLOT(onnewICASourceofInterest(int)));
     ...
     ********************SAVE DATA*******************************************
     ************************************************************************/
    connect(ui->_save_widget,SIGNAL(VideoCreationRequired()),ui->_params_analysis,SIGNAL(VideoCreationRequired()));
    connect(ui->_save_widget,SIGNAL(newVideo_nb_Frames(double)),ui->_params_analysis,SIGNAL(newVideo_nb_Frames(double)));
...
void HMainDisplay::onNewResultDirectory(QString v)
{
    ...
    // if (QFile::exists(v+"Log.txt"))
    //     QFile::remove(v+"Log.txt");

    // //Set debug log files
    // Logger::setLogFile(v+"Log.txt");
    // Logger::install();
...
void HMainDisplay::onNew_Guru_Mode(bool v)
{
    ...
    ui->_params_analysis->onNew_Guru_Mode(v);
...
void HMainDisplay::onNewAutoAnalysisZone(QVector<QPoint> roi,cv::Size s)
{
    //Request image (nb channels) and acquisition (nb frames) infos
    int nb_channels = ui->_params_acquisition->getSpectralCompenentNumber();
    int nb_frames   = ui->_params_acquisition->getAcquisitionFrameNumber();
    ...
    //send ROI to display results class
    ui->_displayResult->setAnalysisZone(roi,s);
...
void HMainDisplay::onNewPlot(QVector<QVector<float> > v)
{
    ...
    //set Y lim axis
    ui->_plot->setYInterval(min,max,0);

    //plot vec
    ui->_plot->setYValue(v,0);
